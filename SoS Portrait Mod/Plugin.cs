...
public class Plugin : BasePlugin
{
    ...
    public class LoadPatch
    {
        ...
        public static void MessagePatch(UISimpleMessage __instance)
        {
            try
            {
                ...
                else
                {
                    ...
                    transform.parent = __instance.transform.parent.transform;
        ...
        private static Sprite ChangeAssets(UITextMeshProOneLine pText, string extra = "")
        { 
            // help from https://forum.unity.com/threads/generating-sprites-dynamically-from-png-or-jpeg-files-in-c.343735/
            Texture2D texture2D;
            byte[] fileBytes;
            var filePath = Path.Combine(SpritePath, pText.text + extra + ".png");
